...
#include <iostream>
#include <sstream>
PointBuildBoard::PointBuildBoard(std::string id, osg::Vec3 selPointPos, osg::ref_ptr<osgViewer::Viewer> viewer, osg::ref_ptr<osg::Camera> camera):osg::Referenced(),mId(id), mSelPointPos(selPointPos), _viewer(viewer), _camera(camera){
...
void OSGPickHandler::Reset() {
	...
	int count = 0;
	if (mOnePoint.valid()) {
		count = mOnePoint->referenceCount();
	...
	if (mOneBoard.valid()) {
		count = mOneBoard->referenceCount();
		mOneBoard.~ref_ptr();
	}
	if (mTwoBoard.valid()) {
		mTwoBoard.~ref_ptr();
